...
class App {
  ...
  async _connectToMQ() {
    ...
    logger.info(`Magneto-Diagnoses : connected to rabbitmq at ${Nconf.get('AMQP_URI')}`);
  ...
  async _closeMQConnection() {
    await MQ.close();
    logger.info(`Magneto-Diagnoses : disconnected from rabbitmq at ${Nconf.get('AMQP_URI')}`);
  }
  ...
  async stop() {
    ...
    logger.info(`Magneto-Diagnoses : shutting down`);
  }
}
