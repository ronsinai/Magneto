...
class App {
  ...
  async _connectToMQ() {
    ...
    logger.info(`Magneto-Diagnoses-Error : connected to rabbitmq at ${Nconf.get('AMQP_URI')}`);
  ...
  async _closeMQConnection() {
    await MQ.close();
    logger.info(`Magneto-Diagnoses-Error : disconnected from rabbitmq at ${Nconf.get('AMQP_URI')}`);
  }
  ...
  async stop() {
    ...
    logger.info(`Magneto-Diagnoses-Error : shutting down`);
  }
}
