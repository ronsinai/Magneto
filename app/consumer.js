...
class App {
  ...
  async _connectToMQ() {
    ...
    logger.info(`Magneto-Discharges : connected to rabbitmq at ${Nconf.get('AMQP_URI')}`);
  ...
  async _closeMQConnection() {
    await MQ.close();
    logger.info(`Magneto-Discharges : disconnected from rabbitmq at ${Nconf.get('AMQP_URI')}`);
  }
  ...
  async stop() {
    ...
    logger.info(`Magneto-Discharges : shutting down`);
  }
}
